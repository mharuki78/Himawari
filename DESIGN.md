...
큰 제목은 Pretendard Variable의 225~250 굵기와 촘촘한 자간으로 세련되고 가벼운 고딕 인상을 만든다. 대표 제목의 의미 있는 마지막 구절은 이끼색 계열로 강조하고, 같은 세 색에서 만든 얕은 단색 오프셋으로 인쇄물 같은 입체감을 준다. 본문은 같은 패밀리의 보통 굵기로 읽기 흐름을 안정시킨다. 한 문단은 최대 `40ch`로 제한하고, 영어 라벨은 작은 크기와 넓은 자간으로만 사용한다. 한 화면의 시선을 주도하는 큰 제목은 하나만 둔다.
