...
- **Brand mark:** `assets/himawari-logo.png`의 공식 로고를 모든 공개 페이지 헤더와 푸터에 사용한다. 밝은 헤더에는 원본 색상을, 어두운 푸터에는 동일 형태의 고대비 단색 처리를 적용한다.
...
헤더 로고는 내비게이션 높이 안에서 가로형 비례가 온전히 읽혀야 하며, 모바일에서도 메뉴와 스토어 버튼 사이에 최소 여백을 유지한다. 푸터 로고는 어두운 바탕에서 동일한 실루엣을 크림색 단색으로 보여준다.
