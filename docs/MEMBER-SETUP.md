...
- Neon Postgres의 `users`, `oauth_accounts`, `member_sessions`, `cart_items`, `wishlist_items`, `orders`, `order_items`, `order_events`를 사용합니다.
...
4. 내부 주문서에서 주문을 접수하고 마이페이지 및 `admin/orders.html`에서 같은 주문번호와 상태가 보이는지 확인합니다.
5. 회원탈퇴 확인 문구를 입력해 로그인 연결·장바구니·관심상품이 삭제되는지 확인합니다. 법정 보존 대상 주문 기록은 회원 프로필과 분리하여 유지됩니다.

연결된 개발 환경에서 `node --env-file=.env.local scripts/smoke-members.mjs`를 실행하면 임시 테스트 계정으로 장바구니·관심상품·탈퇴를 확인하고 테스트 데이터를 즉시 삭제합니다.

현재 주문서는 PG 결제를 실행하지 않고 `결제 대기` 주문만 접수합니다. 네이버페이는 가맹점 계약, 결제 채널 키, 서버 검증과 웹훅이 준비된 뒤 `docs/ORDER-POLICY.md`의 주문 상태와 연결합니다.
