...
test('OAuth 상태 쿠키는 서명·제공자·만료·state를 검증한다', () => {
  ...
  const start = new Request('https://himawari.co.kr/api/auth/start');
  ...
  const callback = new Request(`https://himawari.co.kr/api/auth/callback/naver?state=${created.state}`, {
...
test('설정된 네이버 로그인은 공식 인증 주소와 PKCE와 분리된 state 쿠키를 사용한다', () => {
  ...
  assert.equal(location.searchParams.get('redirect_uri'), 'https://himawari.co.kr/api/auth/callback/naver');
...
test('회원 장바구니와 관심상품 API는 로그인 없는 요청을 거부한다', async () => {
  ...
  process.env.MEMBER_SESSION_SECRET = 'test-secret-that-is-longer-than-thirty-two-characters';
  const cart = await memberHandler(new Request('https://himawari.co.kr/api/member?route=cart'));
  const wishlist = await memberHandler(new Request('https://himawari.co.kr/api/member?route=wishlist'));
