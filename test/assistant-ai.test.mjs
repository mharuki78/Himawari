...
test('AI runs by default, sends only current question, and falls back safely',async()=>{
 ...
 try {
  assert.equal((await (await assistant(req('배송비 알려줘'))).json()).mode,'ai');assert.equal(calls,1);
  assert.equal((await (await assistant(req('배송비 알려줘',true))).json()).mode,'ai');assert.equal(calls,2);
  await assistant(req('배송비 test@example.com',true));await assistant(req('주문번호 HMW-123 배송비',true));await assistant(req('내 배송 조회',true));assert.equal(calls,2);
