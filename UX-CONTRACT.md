...
| Select product-family variant | 제품 카드의 옵션 pill | no network mutation; current card remains stable | same card | image, name, copy, price, stock, wishlist target and purchase/detail destinations switch together | missing image uses the existing card fallback; prior card remains usable | selected option button remains focused | `docs/PRODUCT-CATALOG-POLICY.md` |
...
| Create Npay order | 홈의 각 제품 카드, 상품 상세 또는 장바구니의 `Npay 구매` | SDK 버튼과 등록 요청의 중복 실행 차단, 현재 카탈로그 가격·수량 서버 검증 | 네이버페이 주문서 | 네이버 SDK가 주문서 이동을 소유 | 같은 위치의 라이브 상태로 재시도 안내; 자체 주문서 행동은 유지 | Npay 버튼 영역 | `docs/COMMERCE-POLICY.md`, 네이버페이 가맹점 연동 가이드 2.1 |
