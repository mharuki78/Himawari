...
| Select product-family variant | 제품 카드의 옵션 pill | no network mutation; card geometry stays fixed while the next image preloads | same card | image crossfades, then name, copy, price, stock, wishlist target and purchase/detail destinations remain synchronized | missing image uses the existing card fallback; stale image loads cannot replace a newer selection | selected option button remains focused | `docs/PRODUCT-CATALOG-POLICY.md` |
| Add product to cart | 제품 카드 또는 상세의 `장바구니 담기` | duplicate clicks locked for the feedback interval | same route | count updates once, inline label confirms, and normal-motion users see the product move to the fixed cart control | maximum quantity reports inline without a false success animation; local cart remains usable | initiating control | `DESIGN.md` |
...
| Read product detail | product image/name/`상세 보기` | reserved product loader | `product.html?id=...` | product content, up to three keyboard-operable image points, then an original-ratio continuous detail-image rail | app-owned not-found state and products link; products without highlights omit image points | product title; focused image point announces its matching copy | `docs/PRODUCT-CATALOG-POLICY.md` |
...
| Create Npay order | 상품 상세 또는 장바구니의 `Npay 구매` | SDK 버튼과 등록 요청의 중복 실행 차단, 현재 카탈로그 가격·수량 서버 검증 | 네이버페이 주문서 | 네이버 SDK가 주문서 이동을 소유 | 같은 위치의 라이브 상태로 재시도 안내; 자체 주문서 행동은 유지 | Npay 버튼 영역 | `docs/COMMERCE-POLICY.md`, 네이버페이 가맹점 연동 가이드 2.1 |
| Save Npay wishlist | 상품 상세의 네이버쇼핑 찜 | 같은 상품의 중복 등록 요청 차단 | 네이버 찜 목록 | 네이버 SDK가 찜 목록 이동을 소유 | 같은 위치의 라이브 상태로 재시도 안내; 자사몰 관심상품은 유지 | Npay 버튼 영역 | 네이버페이 가맹점 연동 가이드 2.1 |
...
| Earn game coupon | `게임 시작` then move / jump / slingshot / packing completion | fixed HUD, pause and live score; desktop and mobile keep one locked `100dvh` session; jump avoids hazards and slingshot destroys dumbbells | result panel | score and best active eligible tier stored in current browser | unavailable promotion shows event-preparing state; existing higher reward is retained | result title | `docs/ORDER-POLICY.md` |
