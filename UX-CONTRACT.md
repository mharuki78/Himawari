...
| Earn game coupon | `게임 시작` then catch / packing completion | fixed HUD, pause and live score; mobile keeps one locked `100dvh` session through stage changes | result panel | score and best active eligible tier stored in current browser | unavailable promotion shows event-preparing state; existing higher reward is retained | result title | `docs/ORDER-POLICY.md` |
